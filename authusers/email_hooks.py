...
class AccountDefaultHookSet(object):
    ...
    def send_invitation_email(self, to, ctx):
        subject = render_to_string("authusers/email/invite_user_subject.txt", ctx)
        message = render_to_string("authusers/email/invite_user.html", ctx)
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, to)

    def send_confirmation_email(self, to, ctx):
        subject = render_to_string("authusers/email/email_confirmation_subject.txt", ctx)
        subject = "".join(subject.splitlines())  # remove superfluous line breaks
        message = render_to_string("authusers/email/email_confirmation_message.html", ctx)
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, to)

    def send_password_change_email(self, to, ctx):
        subject = render_to_string("authusers/email/password_change_subject.txt", ctx)
        subject = "".join(subject.splitlines())
        message = render_to_string("authusers/email/password_change.html", ctx)
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, to)

    def send_password_reset_email(self, to, ctx):
        subject = render_to_string("authusers/email/password_reset_subject.txt", ctx)
        subject = "".join(subject.splitlines())
        message = render_to_string("authusers/email/password_reset.html", ctx)
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, to)
